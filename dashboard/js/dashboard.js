...
createApp({
  setup() {
    ...
    // Human-readable file size in Arabic numerals: 245760 -> "٢٤٠ كيلوبايت"
    const formatFileSize = (bytes) => {
      if (bytes >= 1024 * 1024) return `${fmt((bytes / (1024 * 1024)).toFixed(1))} ميجابايت`;
      return `${fmt(Math.max(1, Math.round(bytes / 1024)))} كيلوبايت`;
    };
    ...
    const onImageSelect = (e) => {
      ...
      reader.onload = () => {
        ...
        productForm.imageSize = formatFileSize(file.size);
        productForm.imageDims = '';

        // Read the image's natural dimensions for the metadata row
        const probe = new Image();
        probe.onload = () => {
          productForm.imageDims = `${fmt(probe.naturalWidth)}×${fmt(probe.naturalHeight)} بكسل`;
        };
    ...
    const clearImage = () => {
      ...
      productForm.imageSize = '';
      productForm.imageDims = '';
      const input = document.getElementById('p-image');
      if (input) input.value = '';
    };

    // "Change" button: reopen the file picker directly
    ...
    // 'idle' → 'installing' → 'installed' → 'done' (button removed)
    ...
    onMounted(() => {
      ...
      // Dismiss the loading screen only after ALL page content has fully loaded
      const finish = () => setTimeout(() => { isLoading.value = false; }, 250);
      if (document.readyState === 'complete') finish();
      else window.addEventListener('load', finish, { once: true });
      setTimeout(() => { isLoading.value = false; }, 10000); // safety net
    });
    ...
    return {
      ...
      products, productForm, formError, submitProduct, onImageSelect, clearImage, triggerImagePicker, discountedPrice, hasSizes,
      ...
      logoError, onLogoSelect, clearLogo,
      installState, showInstallConfirm, confirmInstall, fmt,
    };
  },
}).mount('#dashboard');
