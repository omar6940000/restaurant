...
createApp({
  setup() {
    ...
    // Speed: cards render at ~400px, so request 800px crops instead of 1200px
    // (≈40–50% fewer image bytes; hero slides keep their full-width size)
    items.forEach((i) => {
      i.image = i.image.replace('h=627&w=1200', 'h=418&w=800');
    });
    ...
    // Also opt every image into async decoding so large photos never block the main thread.
    ...
    onMounted(() => {
      // The splash screen dismisses only when ALL page content has fully
      // loaded (window 'load': stylesheets, scripts, and every eager image)
      // AND the first hero slide is ready.
      ...
      Promise.all([heroReady, pageLoaded]).then(() => {
        setTimeout(() => { isLoading.value = false; }, 250); // brief grace for paint
      });

      // Safety net: never trap the user on a stalled resource
      setTimeout(() => { isLoading.value = false; }, 10000);

      // Preload the remaining hero slides only AFTER the page is fully loaded,
      // during idle time — they never compete with critical resources
      pageLoaded.then(() => {
        const preloadRest = () => heroSlides.slice(1).forEach((s) => { new Image().src = s.image; });
        if ('requestIdleCallback' in window) requestIdleCallback(preloadRest, { timeout: 3000 });
